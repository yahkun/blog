...
def global_setting(request):
    """
    context_processors: 每个视图函数在渲染数据和模板时，
    都会将此函数的返回值一同渲染
    ...
    # 友情链接数据（同学们自己完成）
    links = Links.objects.all().order_by('-index')
...
def archive(request):
    try:
        ...
        # 直接执行原生sql呢？
        # 第一种方式（不可行）
        # archive_list =Article.objects.raw('SELECT id, DATE_FORMAT(date_publish, "%%Y-%%m") as col_date FROM
        # blog_article ORDER BY date_publish')
        # for archive in archive_list:
        #     print archive
        # 第二种方式（不推荐）
        # cursor = connection.cursor()
        # cursor.execute("SELECT DISTINCT DATE_FORMAT(date_publish, '%Y-%m') as col_date FROM blog_article ORDER BY
        # date_publish")
        # row = cursor.fetchall()
        # print row
        # 先获取客户端提交的信息
